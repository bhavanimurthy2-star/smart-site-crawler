...
class LinkResult:
    ...
    def display_status(self) -> str:
        if self.browser_checked and self.browser_loaded and self.http_status:
            return f"200 (Browser verified; original {self.http_status})"
...
def _classify_link(
    ...
    # aiohttp returned a non-2xx code.  Force FAIL so the browser fallback
    # runs — these platforms reject raw HTTP clients with 404/403 even when
    # the page is live; only a real browser fingerprint reveals the truth.
    if is_reservation_platform(target_url):
        return Verdict.FAIL, "Reservation platform — verifying via browser"
    ...
    if 400 <= code < 500:
        if code in _DEFINITIVE_MISS_CODES:
            ...
            # External 404: browser fallback runs — a WAF may serve the real
            # page to Chromium but reject aiohttp's fingerprint with 404.
            # Internal 404 is a genuine miss; 410/451 are definitively gone.
            if code == 404:
                return (
                    Verdict.FAIL,
                    "Internal 404 — Not Found" if not is_external else "HTTP 404 — Not Found",
                )
...
class LinkValidator:
    ...
    async def _apply_browser_fallback(self, results: list[LinkResult]) -> None:
        ...
        # Running Playwright on every external FAIL would be the main
        # performance bottleneck.  We skip the browser only for codes that
        # are DEFINITIVELY "resource gone" (404 / 410 / 451) — no WAF ever
        # returns those for existing content, so a browser check cannot
        # change the verdict.  Every other external FAIL gets a browser
        # pass because the failure may be WAF fingerprinting, a soft 5xx,
        # or a reservation/bot-blocking platform not in our known lists.
        #
        #   A. Internal links           → always browser (server / deploy bug)
        #   B. External 404 / 410 / 451 → skip browser  (genuinely gone)
        #   C. Everything else external → browser       (WAF / unknown error)
        def _needs_browser(r: LinkResult) -> bool:
            if not r.is_external:
                return True                              # A.
            if r.status_code in _DEFINITIVE_MISS_CODES and r.status_code != 404:
                return False                             # B. 410/451 definitively gone — browser can't help
            if r.status_code == 404:
                return True                              # B2. external 404 — WAF may block aiohttp; verify via browser
            return True                                  # C. possible WAF / soft error
        ...
        if skipped_count:
            logger.info(
                ...
                "(HTTP 410/451 — definitively gone, browser cannot change verdict)",
                skipped_count,
            )
        ...
        for r in fail_candidates:
            ...
            if br.loaded:
                ...
                r.reason       = (
                    "Browser verified (HTTP 404 overridden)"
                    if r.http_status == 404
                    else f"Browser verified — page loads successfully (original HTTP {r.http_status})"
                )
                upgraded += 1
                logger.debug(
